...
WaitingRoomWindow::WaitingRoomWindow(uint8_t maxPlayers, QWidget *parent)
        ...
        // Inicializo slots vacíos
        for (int i = 0; i < max_players; i++) {
            // Se agregan directamente al vector, sin llamar a updatePlayerDisplay aun
            PlayerInfo player{ "Esperando...", "", false, false };
    ...
    void WaitingRoomWindow::addPlayerByName(const QString& name) {
        // 1. Evitar duplicados: si ya existe, actualizamos (puede ser el local)
        if (player_name_to_index.count(name)) {
            int idx = player_name_to_index[name];
            players[idx].name = name; // Asegurarse
            updatePlayerDisplay();
            return;
        }
        ...
        for (size_t i = 0; i < players.size(); i++) {
            if (players[i].name == "Esperando...") {
                ...
                // IMPORTANTE: No resetear isLocal si ya estaba en true (el controlador agrega al local primero)
                // Si es un jugador remoto, isLocal ya estará en false por el constructor
    ...
    void WaitingRoomWindow::removePlayerByName(const QString& name) {
        ...
        // [LOGICA DINAMICA]
        // 1. Eliminar al jugador del vector para que los de abajo suban
        players.erase(players.begin() + indexToRemove);
        
        // 2. Agregar un slot vacío al final para mantener el tamaño maximo
        PlayerInfo emptyPlayer{ "Esperando...", "", false, false };
        players.push_back(emptyPlayer);
        
        // 3. RECONSTRUIR EL MAPA DE INDICES
        // Como borramos un elemento, todos los indices siguientes cambiaron
    ...
    void WaitingRoomWindow::addPlayer(const QString& name, const QString& car, bool isLocal) {
        // Esta funcion auxiliar era usada por el constructor viejo, ahora adaptada
    ...
    void WaitingRoomWindow::setLocalPlayerInfo(const QString& name, const QString& car) {
        // Este metodo no se estaba usando correctamente, ahora addPlayerByName maneja la logica
        addPlayerByName(name);
        setPlayerCarByName(name, car);
        
        // Marcar explicitamente como local si lo encontramos
        if (player_name_to_index.count(name)) {
    ...
    void WaitingRoomWindow::updatePlayerDisplay() {
        ...
        for (int i = 0; i < getCardsPerPage(); i++) {
            ...
            if (playerIdx < static_cast<int>(players.size())) {
                ...
                // Mejor contraste para el jugador local
                ...
                if (player.isLocal) borderColor = "rgb(0, 255, 255)"; // Cyan para local
                ...
                // Mostrar info solo si no es un slot de espera
