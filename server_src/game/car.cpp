...
void Car::load_stats(float max_spd, float accel, float hand, float durability,
...
void Car::turn_left(float delta_time) {
    if (is_destroyed || current_speed < 5.0f) return;  // No girar si está muy lento
...
void Car::turn_right(float delta_time) {
    if (is_destroyed || current_speed < 5.0f) return;
