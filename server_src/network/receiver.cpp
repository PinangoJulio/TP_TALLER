    ...
    try {
            ...
            switch (msg_type) {
                ...
                case MSG_CREATE_GAME: {
                    ...
                    if (current_match_id != -1) {
                        std::cout << "[Receiver] ERROR: " << username << " is already in match " << current_match_id << "\n";
                    ...
                    if (monitor.is_player_in_match(username)) {
                        std::cout << "[Receiver] ERROR: Monitor says " << username << " is already in a match!\n";
                        protocol.send_buffer(LobbyProtocol::serialize_error(ERR_ALREADY_IN_GAME, "You are already in a game (monitor check)"));
                        break;
                    }

                    // Crear la partida (el host se agrega automáticamente)
                    int match_id = monitor.create_match(max_players, username, id, sender_messages_queue);
                ...
                case MSG_LEAVE_GAME: {
                    ...
                    std::cout << "[Receiver] Sent updated games list (" << games.size() << " games)" << std::endl;
        ...
        if (!username.empty() && current_match_id != -1) {
            std::cout << "[Receiver] Cleaning up " << username
                      << " from match " << current_match_id << " (disconnect)" << std::endl;
