...
uint16_t LobbyManager::create_game(const std::string& game_name, const std::string& creator,
                                   uint8_t max_players) {
    ...
    std::cout << "[LobbyManager] Game '" << game_name << "' created with ID " << game_id << " by '"
              << creator << "'" << std::endl;
...
bool LobbyManager::start_game(uint16_t game_id, const std::string& username) {
    ...
    if (!it->second->has_player(username)) {
        std::cout << "[LobbyManager] Player '" << username << "' is not in game " << game_id
                  << std::endl;
        return false;
    }
    ...
    it->second->start();
    std::cout << "[LobbyManager] Game " << game_id << " started by '" << username << "'"
              << std::endl;
...
void LobbyManager::register_player_socket(uint16_t game_id, const std::string& username,
                                          Socket& socket) {
    player_sockets[game_id][username] = &socket;

    std::cout << "[LobbyManager] Registered socket for player '" << username << "' in game "
              << game_id << std::endl;
}
...
void LobbyManager::unregister_player_socket(uint16_t game_id, const std::string& username) {
    ...
    if (it != player_sockets.end()) {
        ...
        std::cout << "[LobbyManager] Unregistered socket for player '" << username << "' in game "
                  << game_id << std::endl;
...
void LobbyManager::broadcast_to_game(uint16_t game_id, const std::vector<uint8_t>& buffer,
    ...
    std::cout << "[LobbyManager] Broadcasting to game " << game_id
              << " (excluding: " << (exclude_username.empty() ? "none" : exclude_username) << ")"
              << std::endl;
