...
int getClipIndex(int dx, int dy) {
    ...
    if (dx == 0 && dy == -1) last_clip = 0;
    else if (dx == 1 && dy == -1) last_clip = 1;
    else if (dx == 1 && dy == 0) last_clip = 2;
    else if (dx == 1 && dy == 1) last_clip = 3;
    else if (dx == 0 && dy == 1) last_clip = 4;
    else if (dx == -1 && dy == 1) last_clip = 5;
    else if (dx == -1 && dy == 0) last_clip = 6;
    else if (dx == -1 && dy == -1) last_clip = 7;
    ...
    try {
        ...
        while (running) {
            ...
            if (player.dir_x != 0 || player.dir_y != 0) {
                ...
                if (nextX < 0) nextX = 0;
                if (nextY < 0) nextY = 0;
                if (nextX >= MAP_WIDTH) nextX = MAP_WIDTH - 1;
                if (nextY >= MAP_HEIGHT) nextY = MAP_HEIGHT - 1;
